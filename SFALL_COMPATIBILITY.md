...
| Perks and traits | get/set_perk_owed<br>set_perk_freq<br>set_perk_image<br>set_perk_*<br>set_pyromaniac_mod<br>apply_heaveho_fix<br>set_swiftlearner_mod<br>has/set_fake_perk<br>has/set_fake_trait<br>set_selectable_perk<br>set_perkbox_title<br>show/hide_real_perks<br>perk_add_mode<br>clear_selectable_perks<br>add/remove_trait<br>seq_perk_freq | implemented: get/set_perk_owed, set_perk_freq, set_perk_image, set_perk_*, has/set_fake_perk, has/set_fake_trait | `ddraw.ini` `PerksFile` supports sfall `Perks.ini` tweaks and existing perk overrides. Named player fake perks/traits appear in the character sheet and persist in sfallgv.sav. Selectable/NPC fake-perk commands and new perks with IDs 119..255 are not yet implemented. |
| Virtual file system | fs_create<br>fs_copy<br>fs_find<br>fs_read/write_*<br>fs_delete<br>fs_size<br>fs_pos<br>fs_seek<br>fs_resize | implemented: fs_create, fs_copy, fs_find, fs_delete, fs_read/write_*, fs_size, fs_pos, fs_seek | Mutable memory files override read-only engine asset lookup; open readers retain stable snapshots. Integer/float byte order and string writes are implemented. RPU walking/Goris scripts rebuild overrides after load. fs_resize and persistent VFS files are not implemented. Engine redirects under sound/sfx remain excluded because SFX metadata is cached before scripts run. |
...
| NPCs | inc_npc_level<br>get_npc_level<br>npc_engine_level_up | implemented: only npc_engine_level_up | - |
| Hero Appearance | set_dm/df_model<br>hero_select_win<br>set_hero_race<br>set_hero_style | implemented: hero_select_win, set_hero_race, set_hero_style | Player-only archive lookup, fallback graphics, saved race/style, inventory FIDs and English selection menu. Android HUD opens the menu outside combat. set_dm/df_model remain unimplemented. Full outfit/animation and EPA scenario coverage is pending; see [RPU tests](docs/rpu-alpha2-tests.md). |
| Events | add_g_timer_event<br>remove_timer_event<br>create_spatial<br>spatial_radius | ✅ | `add_g_timer_event`/`remove_timer_event` also work for regular scripts by using CE's normal script timer queue; sfall documents these as global-script functions. |
